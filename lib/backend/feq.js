...
function OCAfeq(config, options) {
  ...
  // API Paths
  me.path = {
    "checkIn" : { path: "/be", method: "post" },
    "next": { path: "/be", method: "get" },
    "update" : { path: "/be/:id/reject", method: "post", }
  };
  ...
  if (config.clientAuth || config.serverAuth) {
    // Loads the key, cert, and chain into the tlsOptions
    me.tlsOptions = OCAengine.TlsCryptoInit(config);
    // Sets the right options for the serverAuth
  ...
  function __check_fe_connection() {
    ...
    me.OCAquery("post", me.config.baseUrl + "/be", me.tlsOptions, {}, (msg, res) => {
