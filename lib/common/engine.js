...
function OCAlogin(baseUrl, config, callback) {
  ...
  if (config.password != null) {
    ...
    retMsg.err(new OCAErr(500, "Missing credentials type for login."));
    if (typeof callback === "function") return callback(retMsg, null);
    return null;
  }
...
function OCAquery(method, path, options, data, callback ) {
  ...
  options.checkServerIdentity = function $OCAQuery$client$ceckServerIdentity (host, cert) {
  ...
  aRequest.on('error', (err) => {
    if (err != null && err.code != null) {
      OCAlog("[TLS ERROR] Cannot connect to server ['%s://%s:%d'] (code: %s), aborting.", 
        options.protocol, options.hostname, options.port, err.code);
    }
    else if (err != null && err.reason != null) {
      OCAlog("[TLS ERROR] %s, aborting.", err.reason);
    }
    else {
      OCAlog("[TLS ERROR] Cannot connect to server (errObj: %s), aborting.", JSON.stringify(err));
    }
    process.exit(3);
  });
...
function TlsCryptoInit(auth, keyName, certName, caName) {
  ...
  if (certName != null && certName != "") {
    ...
    ret.certName = auth.certAndChain;
  }
