...
function OCAlogin(baseUrl, config, callback) {
  ...
  if (config == null) {
    retMsg.err(new OCAErr(-1, "Missing required ('config') parameter."));
  ...
  if (typeof config.username === "undefined") {
    retMsg.err(new OCAErr(-1, "Missing required ('config.username') parameter."));
    if (typeof callback === "function") return callback(retMsg, null);
    return null;
  }
  ...
  if (typeof config.org === "undefined") {
    retMsg.err(new OCAErr(-1, "Missing required ('config.org') parameter."));
    if (typeof callback === "function") return callback(retMsg, null);
    return null;
  }
  ...
  var loginPath = baseUrl + "/usr/login/" + config.org;
...
function OCAquery(method, path, options, data, callback ) {
  ...
  // Option for not pooling the connection
  // options.agent = false;
  ...
  options.checkServerIdentity = (function(){
    ...
    var parseCertFile = OCAtools.parseCertFile;
    ...
    return function $OCAQuery$client$ceckServerIdentity (host, cert) {
      ...
      function checkConstraints(config, cert, level = 0) {
        ...
        OCAdebug("Checking Server Certificate's Details");

        // Checks if a certificate was set for comparison
        if (config.certName != "" && typeof config.certificate !== "undefined") {
            config.certificate = parseCertFile(config.certName);
        }

        // Checks the certificate's fingerprint
        if (typeof config.certificate !== "undefined" && typeof cert === "undefined") {
          err.reason = "[Level {0}] ERROR: No certificate found".format(level);
          return err;
        } else if (typeof config.certificate !== "undefined" && typeof cert !== "undefined") {
          if (config.certificate.fingerprint != cert.fingerprint.replace(/:/g, '')) {
            err.reason = "[Level {0}] ERROR: Certificate differs from the configured one [cert: {1}, config: {2}]"
              .format(level, cert.fingerprint, config.certificate.fingerprint);
            return err;
          }
        }
        ...
        if (config.subject !== undefined && config.subject != null) {
          ...
          for (var i in config.subject) {
            ...
            if (!cert.subject || !cert.subject[i]) {
              err.reason = "[Level {0}] ERROR: Missing property '{1}' in server's certificate subject"
                .format(level, i);
              return err;
            } else if (cert.subject[i] != config.subject[i]) {
              err.reason = "[Level {0}] ERROR: Attribute [{1}] of the Certificate Subject [value: '{2}', expected: '{3}']"
                .format(level, i, cert.subject[i], config.subject[i]);
              return err;
            }
          }
        }
        ...
        if (config.issuer !== undefined && config.issuer != null) {
          ...
          for (var i in config.issuer) {
            ...
            if (!cert.issuer || !cert.issuer[i]) {
              err.reason = "[Level {0}] ERROR: Missing property '{1}' in server's certificate Issuer"
                .format(level, i);
              return err;
            } else if (cert.issuer[i] != config.issuer[i]) {
              err.reason = "[Level {0}] ERROR: Attribute [{1}] of the Certificate Issuer [value: '{2}', expected: '{3}']"
                .format(level, i, cert.issuer[i], config.issuer[i]);
              return err;
            }
          }
        }
        ...
        if (config.issuerCertificate != null) {
          ...
          if (cert.issuerCertificate == null) {
            err.reason = "ERROR: Constraints found for next chain level, but no next chain certificate was found.";
            return ret;
          }
      ...
      if (err) {
        if (err.host != "localhost" && err.host != "127.0.0.1") {
      ...
      if (typeof options.serverAuth !== "undefined") {
        ...
        var myErr = checkConstraints(srvOpt, cert);
        if (myErr != null) {
          return myErr;
        }
      }
  ...
  aRequest.on('error', (err) => {
    var errMsg = new OCAErr();
    if (err != null && err.code != null) {
      errMsg.errno(err.code);
      errMsg.aux(err);
      errMsg.desc("[TLS ERROR] Cannot connect to server ['{0}://{1}:{2}'] (code: {3}), aborting."
        .format(options.protocol, options.hostname, options.port, err.code));
    }
    else if (err != null && err.reason != null) {
      errMsg.desc("[TLS ERROR] %s, aborting.".format(err.reason));
    }
    else {
      errMsg.desc("[TLS ERROR] Cannot connect to server (errObj: {0}), aborting.".format(JSON.stringify(err)));
    }

    // Closing active connections - we do not want to
    // re-use the connections without re-authentication
    aRequest.abort();
...
function TlsCryptoInit(auth, keyName, certName, caName, srvAuth) {
  ...
  if (keyName != null && keyName != "") {
    ...
    ret.keyName = auth.clientAuth.privKey;
  }
